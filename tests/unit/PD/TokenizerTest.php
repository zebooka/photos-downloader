...
class TokenizerTest extends TestCase
{
    ...
    private function configure(): Configure
    {
        return \Mockery::mock(Configure::class)
            ->shouldReceive('knownAuthors')->withNoArgs()->andReturn(array('BOA', 'CHV'))->getMock()
            ->shouldReceive('knownCameras')->withNoArgs()->andReturn(array('k100d', 'lx5', '5s'))->getMock()
            ->shouldReceive('knownTokens')->withNoArgs()->andReturn(array('hello', 'old', 'world'))->getMock();
    }
    ...
    public function test_tokenize()
    {
        ...
        $tokenizer = new Tokenizer(
            $this->configure(),
            $this->input([
                Command::TOKENS_ADD => ['new'],
                Command::TOKENS_DROP => ['old'],
                Command::TOKENS_DROP_UNKNOWN => true,
            ]),
            $this->exifAnalyzer($fileBunch, null, null)
        );
    ...
    {
        $fileBunch = $this->fileBunch('S_BOA_hello_070417_210000,2_k100d_old_world_old_unknown');
        $tokenizer = new Tokenizer($this->configure(), $this->input(), $this->exifAnalyzer($fileBunch, null, null));
        $tokens = $tokenizer->tokenize($fileBunch);
        $this->assertInstanceOf(Tokens::class, $tokens);
        $this->assertEquals('BOA', $tokens->author);
    ...
    public function test_tokenize_dcim_photo()
    {
        ...
        $tokenizer = new Tokenizer($this->configure(), $this->input(), $this->exifAnalyzer($fileBunch, strtotime('2007-04-17 21:00:00'), 'unique-camera'));
        $tokens = $tokenizer->tokenize($fileBunch);
        $this->assertInstanceOf(Tokens::class, $tokens);
        $this->assertEquals('070417', $tokens->date());
        $this->assertEquals('210000', $tokens->time());
    ...
    public function test_tokenize_failure_with_incorrect_dates_with_placeholders()
    {
        ...
        $tokenizer = new Tokenizer($this->configure(), $this->input(), $exifAnalyzer);
        $tokenizer->tokenize($fileBunch);
    }
    ...
    public function test_tokenize_preferExifDateTime()
    {
        ...
        $tokenizer = new Tokenizer($this->configure(),
            ...
            $this->exifAnalyzer($fileBunch, '2009-08-21 11:00:00', null));
        $tokens = $tokenizer->tokenize($fileBunch);
        $this->assertInstanceOf(Tokens::class, $tokens);
    ...
    public function test_tokenize_skips_img_dsc_token()
    {
        foreach (array('IMG', 'DSC', 'DJI', 'ANY') as $notAuthor) {
            ...
            $tokenizer = new Tokenizer($this->configure(), $this->input(), $this->exifAnalyzer($fileBunch, '2015-11-01 22:00:00', null));
            $tokens = $tokenizer->tokenize($fileBunch);
            $this->assertInstanceOf(Tokens::class, $tokens);
            $this->assertNull($tokens->author);
